<?php
	...
	while ($eleve = $pseleves->fetch()) {
		if ($eleve['ID'] == $_SESSION['PROFILE']['LIEN_ELEVE']) {
			$classe_user = $eleve['CLASSE'];
			$req = "SELECT * FROM messages WHERE DESTINATAIRE_CLASSE=$classe_user OR DESTINATAIRE_ID=$user_id ORDER BY ID DESC";
	...
	$req = "SELECT * FROM messages WHERE DESTINATAIRE_ID=$user_id ORDER BY ID DESC";
	$ps = $pdo->prepare($req); //objet PDO et prepare statement
	$ps->execute();
	}
...
<body>
	...
	<h3 class="w3-opacity bold_style w3-text-white">VOS MESSAGES</h3>
	</div>
	...
	<table class="w3-table-all w3-hoverable">
		<thead>
		<tr class="w3-grey">
                <th class="w3-center">NOM</th><th class="w3-center">TITRE</th><th class="w3-center">MESSAGE</th><th class="w3-center">DESTINATAIRE</th><th class="w3-center">HEURE</th>			</tr>
		</thead>
		<tbody>

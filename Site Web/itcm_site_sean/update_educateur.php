...
if($nomPhoto != "") {
	$nomPhoto1 = $id . $nom . $prenom . "AvatarEduc" . $nomPhoto;
	$fichierTempo=$_FILES['photo']['tmp_name'];
	move_uploaded_file($fichierTempo,"./images/$nomPhoto1");
	$ps101=$pdo->prepare("UPDATE educateurs SET NOM=?,PRENOM=?,EMAIL=?,PHOTO=? WHERE ID=?");
	$params101=array($nom,$prenom,$email,$nomPhoto1,$id);
	$ps101->execute($params101);
}

else {
	$ps100 = $pdo->prepare("UPDATE educateurs SET NOM=?,PRENOM=?,EMAIL=? WHERE ID=?");

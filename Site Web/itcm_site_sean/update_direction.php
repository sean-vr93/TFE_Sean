...
if($nomPhoto == "") {
	$ps100 = $pdo->prepare("UPDATE directions SET NOM=?,PRENOM=?,EMAIL=? WHERE ID=?");
	$params100=array($nom,$prenom,$email,$id);
	$ps100->execute($params100);
}
if($nomPhoto != "") {
	$nomPhoto1 = $id . $nom . $prenom . "AvatarDirec" . $nomPhoto;
	$fichierTempo=$_FILES['photo']['tmp_name'];
	move_uploaded_file($fichierTempo,"./images/$nomPhoto1");
	$ps101=$pdo->prepare("UPDATE directions SET NOM=?,PRENOM=?,EMAIL=?,PHOTO=? WHERE ID=?");
